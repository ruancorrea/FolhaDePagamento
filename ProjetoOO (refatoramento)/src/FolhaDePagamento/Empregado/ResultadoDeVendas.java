...
public class ResultadoDeVendas {
    public static void ResultadoVendas(Empresa P3, int i, Empresa[] undoredo) {
        ...
        if(Lista[i] instanceof FolhaDePagamento.Empregado.Comissionado)
        {
            ...
            FolhaDePagamento.Empregado.Funcionario F = new FolhaDePagamento.Empregado.Comissionado(Lista[i].getNome(), Lista[i].getEndereco(), Lista[i].getID(), Lista[i].getSindicato(),
                    Lista[i].getSindicatoID(), Lista[i].getSalario(), Lista[i].getTaxaSindical(), Lista[i].getMetodo(), Lista[i].getPagamento(),
                    Lista[i].getAgenda(), Lista[i].getNasemana(), P3.getDay(), P3.getMonth(), P3.getYear(), ((FolhaDePagamento.Empregado.Comissionado)Lista[i]).getDiastrabalhados(), salarioatual, Lista[i].isBateuPonto(),
